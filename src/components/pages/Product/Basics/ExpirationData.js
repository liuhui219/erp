...
export default class Card extends React.Component {
    ...
    edit = (record) => {
        ...
        var staffData = [{
            ...
            data:{
                key:'expirationUtil',
                value:JSON.stringify(record.expirationUtil),
                child:[{id:'1',text:'年'},{id:'2',text:'月'},{id:'3',text:'日'}]
            },
            inputCenter:{
                name: '保质期	',
                key:'expirationValue',
                massage: '请填写保质期',
                value:record.expirationValue,
                must: true
            }
        }, {
            name: '描述',
    ...
    list = () => {
        ...
        fetch(globals.url.url + '/product/expirationDate/list', {
            method: 'get',
            credentials: 'include'
        })
    ...
    freeze(record) {
        ...
        if(record.freezeStatus == 1){
            url = globals.url.url + '/product/expirationDate/freeze?id=' + record.id;
        }else{
            url = globals.url.url + '/product/expirationDate/thaw?id=' + record.id;
        }
        fetch(url, {
            method: 'GET',
            credentials: 'include',
    ...
    handleCreate = () => {
        ...
        form.validateFields((err, values) => {
            ...
            if (DataType.type == 'add') {
                url = '/product/expirationDate/add';
            } else {
                url = '/product/expirationDate/update';
            }
    ...
    render() {
        ...
        const columns = [
            ...
            { title: '保质期', dataIndex: 'expirationValue1', width: 150, key: 'expirationValue1' , render: (text, record, index) => 
                    <span>
                        {record.expirationValue}
                        {record.expirationUtil ==1 && '年'}
                        {record.expirationUtil ==2 && '月'}
                        {record.expirationUtil ==3 && '日'}
                    </span>
            },
