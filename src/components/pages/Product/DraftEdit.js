...
class DraftEdit extends Component {
    ...
    if(this.state.headResourceKey == ''){
      ...
      this.props.form.validateFields((err, values) => {
        if (!err) {
          ...
          fetch(globals.url.url+'/product/draft/update', {
            method: 'POST',
            credentials: 'include',
            headers: {
              'Content-Type': 'application/json'
            ...
            if(result.code == 0){
              ...
              that.props.history.push({pathname: '/erp/pro-draft-info/' + that.props.match.params.id});
              var obj = {name:'产品详情',menuKey:'pro-draft-info/'+ that.props.match.params.id,key:'pro-draft-info'};
              that.props.dispatch({ type: 'INCREMENT' ,text:obj});
              var objs = {menuKey:'pro-Draft-edit',key:'pro-Draft-edit'};
    ...
    if(this.state.headResourceKey == ''){
      ...
      this.props.form.validateFields((err, values) => {
        if (!err) {
          ...
          fetch(globals.url.url+'/product/draft/add', {
            method: 'POST',
            credentials: 'include',
            headers: {
              'Content-Type': 'application/json'
            ...
            if(result.code == 0){
              ...
              var objs = {menuKey:'pro-Draft-edit',key:'pro-Draft-edit'};
              that.props.dispatch({ type: 'header' ,text:objs});
              var objsr = {menuKey:'pro-draft-info',key:'pro-draft-info'};
              that.props.dispatch({ type: 'header' ,text:objsr});
            }else{
              message.warning(result.message);
              that.setState({
                spinning:false,
  ...
  close = (e) => {
    this.props.history.push({pathname: '/erp/pro-draft-info/' + this.props.match.params.id});
           ...
           <Form className="page_from" style={{width: '100%'}} onSubmit={this.handleSubmit}>
              <div className="page_add_input_btn">
                 ...
                 <div className="page_add_input_btn_right">
                     ...
                     <Button type="primary" onClick={this.submits} icon="reload">
                       提交
                     </Button>
                 </div>
              </div>
...
export default Form.create()(connect()(DraftEdit));
