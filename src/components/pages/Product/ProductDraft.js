...
class ProductDraft extends Component {
  constructor() {
    super();
    this.state={
      data:[],
  ...
  getData(page,num){
    var that = this;
    fetch(globals.url.url+'/product/draft/list?pageNumber='+page+'&pageSize='+num, {
      method: 'GET',
      credentials: 'include'
...
toQueryString(obj) {
  ...
  reloads(data){
    ...
    fetch(globals.url.url+'/product/draft/updateToCommit?id='+data, {
      method: 'POST',
      credentials: 'include'
    }).then(function(response) {
      return response.json();
    ...
    const columns = [{
      ...
      render: (text, record,i) => (
               <div style={{display:'flex',alignItems:'center'}}>
                   <Tooltip title="提交">
                       <span onClick={this.reloads.bind(this,record.id)} className="account-table-title" style={{cursor:'pointer',marginTop:3,}}>
                           <Icon type="reload" style={{fontSize:18,color:'#428ef2'}} />
                       </span>
                   </Tooltip>
                   <Tooltip title="删除">
                       <span onClick={this.delete.bind(this,Array.of(record.id))} className="account-table-title" style={{cursor:'pointer',marginLeft:10,marginTop:3,}}>
                           <Icon type="close-circle" style={{fontSize:18,color:'#428ef2'}} />
                       </span>
                   </Tooltip>
                   <Tooltip title="查看详情">
...
export default connect()(ProductDraft);
