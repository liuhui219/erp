...
class RecordInfo extends Component {
  ...
  deletes = (e) => {
    ...
    fetch(globals.url.url+'/product/product/updateToFreeze', {
      ...
      if(result.code == 0){
         ...
         that.props.dispatch({ type: 'INCREMENT' ,text:obj});
         var objs = {menuKey:'pro-Record-info',key:'pro-Record-info'};
  ...
  edits = (e) => {
    this.props.history.push({pathname: '/erp/pro-record-edit/' + this.props.match.params.id});
    var obj = {name:'产品档案修改',menuKey:'pro-record-edit/'+ this.props.match.params.id,key:'pro-record-edit'};
    this.props.dispatch({ type: 'INCREMENT' ,text:obj});
  }

  copy = (e) => {
    this.props.history.push({pathname: '/erp/pro-add-record/' + this.props.match.params.id});
           ...
           <Form className="page_from" style={{width: '100%'}} onSubmit={this.handleSubmit}>
              <div className="page_add_input_btn">
                 ...
                 <div className="page_add_input_btn_right">
                     ...
                     <Button type="primary" onClick={this.copy} icon="copy">
                       复制
                     </Button>
                     {this.state.mains.freezeStatus == 1 ? <Button type="primary" onClick={this.deletes} icon="lock">
                       冻结
                     </Button> : null}
                     {this.state.mains.freezeStatus == 2 ? <Button type="primary" onClick={this.undeletes} icon="unlock">
                       解冻
                     </Button> : null}
                 </div>
              </div>
...
export default Form.create()(connect()(RecordInfo));
