...
export default class Card extends React.Component {
    ...
    edit = (record) => {
        ...
        var staffData = [{
            ...
            tag: 'select',
            value:record.name,
            data:[{id:'增值税专用发票',categoryName:'增值税专用发票'},{id:'增值税普通发票',categoryName:'增值税普通发票'}],
            must: true
        },{
            name: '系统内置',
            key: 'isSystem',
    ...
    // 删除
    delete = (id) => {
        this.save('', '/hr/tax/delete?id=' + id)
    }
    // 获取列表
    list = () => {
        let that = this;
        let data = that.state.data;
        fetch(globals.url.url + '/fin/tax/list', {
            method: 'get',
            credentials: 'include'
        })
    ...
    freeze(record) {
        ...
        if(record.freezeStatus == 1){
            url = globals.url.url + '/fin/tax/updateDisable?id=' + record.id;
        }else{
            url = globals.url.url + '/fin/tax/updateEnable?id=' + record.id;
        }
        fetch(url, {
            method: 'GET',
            credentials: 'include',
    ...
    render() {
        ...
        const columns = [
            ...
            { title: '税率', dataIndex: 'rate', width: 150, key: 'rate' },
            { title: '税种', dataIndex: 'name', width: 150, key: 'name' },
