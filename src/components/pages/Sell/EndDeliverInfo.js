...
var mains = [];
var subData = [];
class EndDeliverInfo extends Component {
  constructor() {
    super();
    this.state={
      tip:'加载中...',
  ...
  componentDidMount(){
    this.getData();
    this.props.history.push({pathname: '/erp/sale-logistics-EndDeliverInfo/' +this.props.match.params.id});
    var obj = {name:'已完成出货单详情',menuKey:'sale-logistics-EndDeliverInfo/'+ this.props.match.params.id,key:'sale-logistics-EndDeliverInfo'};
    this.props.dispatch({ type: 'INCREMENT' ,text:obj});
  }

  getData(){
    var that = this;
    fetch(globals.url.url+'/sale/outstock/showComplete?id='+this.props.match.params.id, {
      method: 'GET',
      credentials: 'include'
...
export default Form.create()(connect()(EndDeliverInfo));
