...
var mains = [];
var subData = [];
class UndeliveryInfo extends Component {
  constructor() {
    super();
    this.state={
      tip:'加载中...',
  ...
  componentDidMount(){
    this.getData();
    this.getSelect();
    this.props.history.push({pathname: '/erp/sale-UndeliveryInfo/' +this.props.match.params.id});
    var obj = {name:'订单详情',menuKey:'sale-UndeliveryInfo/'+ this.props.match.params.id,key:'sale-UndeliveryInfo'};
    this.props.dispatch({ type: 'INCREMENT' ,text:obj});
  }

  getData(){
    var that = this;
    fetch(globals.url.url+'/sale/order/show?id='+this.props.match.params.id, {
      method: 'GET',
      credentials: 'include'
           ...
           <Form className="page_from" style={{width: '100%'}} onSubmit={this.handleSubmit}>
              <div className="page_add_input_btn">
                 ...
                 <div className="page_add_input_btn_right">
                     ...
                     <Button type="primary" onClick={this.showModal} icon="upload">
                       出货
                     </Button>
...
export default Form.create()(connect()(UndeliveryInfo));
