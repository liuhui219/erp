...
class Waitdelivery extends Component {
  constructor() {
    super();
    this.state={
      data:[],
  ...
  getData(page,num){
    var that = this;
    fetch(globals.url.url+'/sale/outstock/listOnNotify?pageNumber='+page+'&pageSize='+num, {
      method: 'GET',
      credentials: 'include'
...
toQueryString(obj) {
  ...
  reloads(id){
    this.props.history.push({pathname: '/erp/sale-WaitdeliveryInfo/' +id});
    var obj = {name:'出货通知单详情',menuKey:'sale-WaitdeliveryInfo/'+ id,key:'sale-WaitdeliveryInfo'};
      ...
      <div className="page_check">
         <Spin size="large" tip={this.state.tip} spinning={this.state.spinning}>
             ...
             <div className="page_table">
                <Table rowSelection={rowSelection} columns={columns}  dataSource={this.state.data} scroll={{x:1400,y:'calc(100% - 60px)'}} bordered pagination={false}/>
             </div>
...
export default connect()(Waitdelivery);
