...
var mains = [];
var subData = [];
class OrderRefuseEdit extends Component {
  constructor() {
    super();
    this.state={
      tip:'加载中...',
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        fetch(globals.url.url+'/sale/order/updateRejectOrder', {
          method: 'POST',
          credentials: 'include',
          headers: {
            'Content-Type': 'application/json'
          ...
          if(result.code == 0){
            ...
            that.props.history.push({pathname: '/erp/sale-OrderRefuseInfo/' +that.props.match.params.id});
            var obj = {name:'驳回详情',menuKey:'sale-OrderRefuseInfo/'+ that.props.match.params.id,key:'sale-OrderRefuseInfo'};
            that.props.dispatch({ type: 'INCREMENT' ,text:obj});
            var objs = {menuKey:'sale-orderRefuseEdit',key:'sale-orderRefuseEdit'};
...
export default Form.create()(connect()(OrderRefuseEdit));
