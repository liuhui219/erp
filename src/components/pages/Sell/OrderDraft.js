...
class OrderDraft extends Component {
  ...
  getData(page,num){
    var that = this;
    fetch(globals.url.url+'/sale/orderDraft/list?pageNumber='+page+'&pageSize='+num, {
      method: 'GET',
      credentials: 'include'
...
toQueryString(obj) {
  ...
  delete(data){
    ...
    if(data.length == 0){
      message.warning('请选择要删除的列表');
        ...
        fetch(globals.url.url+'/sale/orderDraft/delete', {
          method: 'POST',
          credentials: 'include',
          headers: {
  ...
  reloads(id){
    this.props.history.push({pathname: '/erp/sale-OrderDraftInfo/' +id});
    var obj = {name:'订单详情',menuKey:'sale-OrderDraftInfo/'+ id,key:'sale-OrderDraftInfo'};
...
export default connect()(OrderDraft);
