...
var mains = [];
var subData = [];
class OrderCheckInfo extends Component {
  constructor() {
    super();
    this.state={
      tip:'加载中...',
  ...
  componentDidMount(){
    this.getData();
    this.getSelect();
    this.props.history.push({pathname: '/erp/sale-OrderCheckInfo/' +this.props.match.params.id});
    var obj = {name:'审核详情',menuKey:'sale-OrderCheckInfo/'+ this.props.match.params.id,key:'sale-OrderCheckInfo'};
    this.props.dispatch({ type: 'INCREMENT' ,text:obj});
  }

  getData(){
    var that = this;
    fetch(globals.url.url+'/sale/order/showChecking?id='+this.props.match.params.id, {
      method: 'GET',
      credentials: 'include'
  ...
  handleOks = () =>{
    ...
    fetch(globals.url.url+'/sale/order/updateCheckStatus?status=3&rejectRemark='+that.state.input, {
      method: 'POST',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/JSON'
      ...
      if(result.code == 0){
         ...
         that.props.history.push({pathname: '/erp/sale-order-check'});
         var obj = {name:'订单审核',menuKey:'sale-order-check',key:'sale-order-check'};
         that.props.dispatch({ type: 'INCREMENT' ,text:obj});
         var objs = {menuKey:'sale-OrderCheckInfo',key:'sale-OrderCheckInfo'};
  ...
  edit = (e) => {
    ...
    var obj = {name:'订单详情',menuKey:'sale-EditOrderDraft/'+ this.props.match.params.id,key:'sale-EditOrderDraft'};
    this.props.dispatch({ type: 'INCREMENT' ,text:obj});
  }
...
export default Form.create()(connect()(OrderCheckInfo));
