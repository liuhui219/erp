...
class OrderRefuse extends Component {
  constructor() {
    super();
    this.state={
      data:[],
  ...
  getData(page,num){
    var that = this;
    fetch(globals.url.url+'/sale/order/listReject?pageNumber='+page+'&pageSize='+num, {
      method: 'GET',
      credentials: 'include'
...
toQueryString(obj) {
  ...
  reloads(id){
    this.props.history.push({pathname: '/erp/sale-OrderRefuseInfo/' +id});
    var obj = {name:'驳回详情',menuKey:'sale-OrderRefuseInfo/'+ id,key:'sale-OrderRefuseInfo'};
  ...
  render() {
    const columns = [{
      ...
      render: (text, record,i) => (
                <div style={{display:'flex',alignItems:'center'}}>
                    <Popconfirm title="确定删除?" onConfirm={this.delete.bind(this,Array.of(record.id))}  okText="确定" cancelText="取消">
                        <Tooltip title="删除">
                    ...
                    <Popconfirm title="确定提交?" onConfirm={this.sure.bind(this,Array.of(record.id))}  okText="确定" cancelText="取消">
                        <Tooltip title="重新提交">
...
export default connect()(OrderRefuse);
