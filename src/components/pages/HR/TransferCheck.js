...
class TransferCheck extends Component {
  constructor() {
    super();
    this.state={
      data:[],
  ...
  getData(page,num){
    var that = this;
    fetch(globals.url.url+'/hr/transfer/listPend?pageNum='+page+'&pageSize='+num, {
      method: 'GET',
      credentials: 'include'
...
toQueryString(obj) {
  ...
  StaffInfo(data){
    this.props.history.push({pathname: '/erp/hr-transferjob-TransferCheckInfo/' + data.id});
    var obj = {name:'调岗详情-'+data.employeeName,menuKey:'hr-transferjob-TransferCheckInfo/'+ data.id,key:'hr-transferjob-TransferCheckInfo'};
...
export default connect()(TransferCheck);
