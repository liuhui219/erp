...
class TransferHandle extends Component {
  constructor() {
    super();
    this.state={
      data:[],
  ...
  getData(page,num){
    var that = this;
    fetch(globals.url.url+'/hr/transfer/listPass?pageNum='+page+'&pageSize='+num, {
      method: 'GET',
      credentials: 'include'
...
toQueryString(obj) {
  ...
  StaffInfo(data){
    this.props.history.push({pathname: '/erp/hr-transferjob-transferOperation/' + data.id});
    var obj = {name:'调岗办理-'+data.employeeName,menuKey:'hr-transferjob-transferOperation/'+ data.id,key:'hr-transferjob-transferOperation'};
...
export default connect()(TransferHandle);
