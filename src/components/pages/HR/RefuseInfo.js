...
class RefuseInfo extends Component {
  constructor() {
    super();
    this.state={
      data:{},
  ...
  getData(){
    ...
    fetch(globals.url.url+'/hr/employee/showPend?id='+this.props.match.params.id, {
      ...
      if(result.code == 0){
        ...
        var obj = {name:'驳回详情-'+result.result.name,menuKey:'hr-entryjob-RefuseInfo/'+ that.props.match.params.id,key:'hr-entryjob-RefuseInfo'};
  ...
  handleOk(){
    ...
    fetch(globals.url.url+'/hr/employee/updatePend', {
      ...
      if(result.code == 0){
         ...
         message.success('已提交');
         history.push({pathname: '/erp/hr-entryjob-refuse',});
         var obj = {menuKey:'hr-entryjob-RefuseInfo',key:'hr-entryjob-RefuseInfo'};
         that.props.dispatch({ type: 'header' ,text:obj});
      }else{
        message.warning(result.message);
        that.setState({
          spinning:false
  ...
  sure(){
    ...
    fetch(globals.url.url+'/hr/employee/delete', {
      ...
      if(result.code == 0){
         ...
         message.success('已删除');
         that.props.history.push({pathname: '/erp/hr-entryjob-refuse',});
         var obj = {menuKey:'hr-entryjob-RefuseInfo',key:'hr-entryjob-RefuseInfo'};
         that.props.dispatch({ type: 'header' ,text:obj});
      }else{
        message.warning(result.message);
  ...
  revise(){
    this.props.history.push({pathname: '/erp/hr-entryjob-Revise/' + this.props.match.params.id});
          ...
          <div className='page_add_input' style={{padding:20}}>
              ...
              {this.state.show ? <div className="page_infos">
                 <div className="page_infos_main">
                    <li>{this.state.data.hiredate}入职</li>
                 ...
                 <div className="page_infos_main">
                    <li>邮箱：{this.state.data.email}入职</li>
...
export default connect()(RefuseInfo);
