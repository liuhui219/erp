...
class revise extends Component {
  ...
  componentDidMount(){
    this.getData();
    this.getAll();
    var obj = {name:'入职驳回-修改',menuKey:'hr-entryjob-Revise/'+ this.props.match.params.id,key:'hr-entryjob-Revise'};
    this.props.dispatch({ type: 'INCREMENT' ,text:obj});
  }

  getAll(){
    var that = this;
    fetch(globals.url.url+'/hr/employee/showReject?id='+this.props.match.params.id, {
      method: 'GET',
      credentials: 'include'
 ...
 send(obj){
   var that = this;
   fetch(globals.url.url+'/hr/employee/updateEmplReject', {
     method: 'POST',
     credentials: 'include',
     headers: {
     ...
     if(result.code == 0){
       ...
       that.props.history.push({pathname: '/erp/hr-entryjob-RefuseInfo/' + that.props.match.params.id});
       var obj = {name:'驳回详情',menuKey:'hr-entryjob-RefuseInfo/'+ that.props.match.params.id,key:'hr-entryjob-RefuseInfo'};
       that.props.dispatch({ type: 'INCREMENT' ,text:obj});
       var objs = {menuKey:'hr-entryjob-Revise',key:'hr-entryjob-Revise'};
 ...
 check = (rule, value, callback) => {
  ...
  cancel(){
    this.props.history.push({pathname: '/erp/hr-entryjob-RefuseInfo/' + this.props.match.params.id});
    var obj = {name:'驳回详情-'+this.state.dataAll.name,menuKey:'hr-entryjob-RefuseInfo/'+ this.props.match.params.id,key:'hr-entryjob-RefuseInfo'};
    this.props.dispatch({ type: 'INCREMENT' ,text:obj});
    var objs = {menuKey:'hr-entryjob-Revise',key:'hr-entryjob-Revise'};
...
export default Form.create()(connect()(revise));
