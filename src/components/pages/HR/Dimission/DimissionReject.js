...
class DepartureAudit extends Component {
  ...
  getData(page,num){
    var that = this;
    fetch(globals.url.url+'/hr/dimission/rejectPageList?checkStatus=3&pageNum='+page+'&pageSize='+num, {
      method: 'GET',
      credentials: 'include'
...
toQueryString(obj) {
  ...
  StaffInfo(data){
    this.props.history.push({pathname: '/erp/DimissionRejectInfo/' + data.id});
    var obj = {name:'离职驳回-'+data.employeeName,menuKey:'DimissionRejectInfo/'+ data.id,key:'DimissionRejectInfo'};
    ...
    const columns = [{
      ...
      render: (text, record,i) => (
               <div style={{display:'flex',justifyContent:'center',alignItems:'center'}}>
                   <Popconfirm title="你确定重新提交吗？" onConfirm={this.reject.bind(this,record.id)}  okText="确定" cancelText="取消">
                        <Tooltip title="重新提交">
