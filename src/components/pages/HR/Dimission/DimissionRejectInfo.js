...
class TransferCheckInfo extends Component {
  ...
  sure(type){
    ...
    let hint = '';
    if(type == 'delete'){
      url = globals.url.url+'/hr/dimission/delete';
      hint = '删除成功';
    }else{
      url = globals.url.url+'/hr/dimission/resubmit';
      hint ='重新提交成功'
    }
    fetch(url, {
      method: 'POST',
      ...
      if(result.code == 0){
         ...
         that.props.history.push({pathname: '/erp/hr-outjob-refuse'});
         var obj = {name:'离职驳回',menuKey:'hr-outjob-refuse',key:'hr-outjob-refuse'};
         that.props.dispatch({ type: 'INCREMENT' ,text:obj});
      }else{
        message.warning(result.message);
        that.setState({
  ...
  edit(){
    let that = this;
    that.props.history.push({pathname: '/erp/DimissionRejecteAmend/'+that.state.main.id});
    var obj = {name:'离职驳回修改-'+that.state.main.employeeName,menuKey:'DimissionRejecteAmend',key:'DimissionRejecteAmend'};
    that.props.dispatch({ type: 'INCREMENT' ,text:obj});
  }
  onChangeInput(e){
    this.setState({
        ...
        <div className='page_add_input' style={{height:'100%'}}>
          <div className="page_from" style={{width: '100%'}}>
            <div className="page_add_input_btn">
               <li style={{fontSize:14,color:'#333'}}>离职驳回</li>
               <div className="page_add_input_btn_right">
                  <Button type="primary" onClick={this.sure.bind(this,'submit')} icon="close-square-o">
                     重新提交 
                   </Button>
                   <Button type="primary" onClick={this.sure.bind(this,'delete')} icon="close-square-o">
                     删除 
                   </Button>
                   <Button type="primary" onClick={this.edit.bind(this)} htmlType="submit" icon="save">
                     修改
                   </Button>
               </div>
            </div>
