...
class TransferRecord extends Component {
  ...
  getData(page,num){
    var that = this;
    fetch(globals.url.url+'/hr/transfer/listOver?pageNum='+page+'&pageSize='+num, {
      method: 'GET',
      credentials: 'include'
...
toQueryString(obj) {
  ...
  StaffInfo(data){
    this.props.history.push({pathname: '/erp/hr-transferjob-TransferRecordInfo/' + data.id});
    var obj = {name:'调岗档案-'+data.employeeName,menuKey:'hr-transferjob-TransferRecordInfo/'+ data.id,key:'hr-transferjob-TransferRecordInfo'};
...
export default connect()(TransferRecord);
