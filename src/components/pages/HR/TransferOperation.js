...
class TransferOperation extends Component {
  ...
  getData(){
    ...
    fetch(globals.url.url+'/hr/transfer/showPass?id='+this.props.match.params.id, {
      ...
      if(result.code == 0){
        ...
        var obj = {name:'调岗办理-'+result.result.employeeName,menuKey:'hr-transferjob-transferOperation/'+ that.props.match.params.id,key:'hr-transferjob-transferOperation'};
  ...
  modalSend(obj){
    ...
    fetch(globals.url.url+'/hr/transfer/updateTranOver', {
      ...
      if(result.code == 0){
        ...
        that.props.history.push({pathname: '/erp/hr-transferjob-handle'});
        var obj = {name:'调岗办理',menuKey:'hr-transferjob-handle',key:'hr-transferjob-handle'};
        that.props.dispatch({ type: 'INCREMENT' ,text:obj});
        var objs = {menuKey:'hr-transferjob-transferOperation',key:'hr-transferjob-transferOperation'};
  ...
  saves(){
    if(!this.state.main.transferGoodsEmployeeName){
      message.warning('请先添加调岗办理');
    }else{
      var that = this;
      var obj = {
        "transferGoodsEmployeeId": that.state.main.transferGoodsEmployeeId,
    		"transferGoodsTime": that.state.main.transferGoodsTime,
    		"transferGoodsType": that.state.main.transferGoodsType,
    		"transferWorkEmployeeId": that.state.main.transferWorkEmployeeId,
    		"transferWorkTime": that.state.main.transferWorkTime,
    		"transferWorkType": that.state.main.transferWorkType,
        'id': Number(that.props.match.params.id),
        'employeeId': Number(that.state.main.employeeId),
        'newDepartmentId': that.state.main.newDepartmentId,
        'newPositionId':that.state.main.newPositionId
      }
      fetch(globals.url.url+'/hr/transfer/updateTranOver', {
        method: 'POST',
        credentials: 'include',
...
export default connect()(TransferOperation);
