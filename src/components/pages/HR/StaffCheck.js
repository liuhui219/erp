...
class StaffCheck extends Component {
  constructor() {
    super();
    this.state={
      data:[],
  ...
  getData(page,num){
    var that = this;
    fetch(globals.url.url+'/hr/employee/listPend?pageNum='+page+'&pageSize='+num, {
      method: 'GET',
      credentials: 'include'
...
toQueryString(obj) {
  ...
  StaffInfo(data){
    this.props.history.push({pathname: '/erp/hr-entryjob-info/' + data.id});
    var obj = {name:'审核详情-'+data.name,menuKey:'hr-entryjob-info/'+ data.id,key:'hr-entryjob-info'};
...
export default connect()(StaffCheck);
