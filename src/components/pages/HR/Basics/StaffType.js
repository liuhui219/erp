...
export default class StaffType extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
    ...
    // 删除
    delete = (id) => {
        this.save('', '/hr/employeeType/delete?id=' + id)
    }
    // 获取列表
    list = () => {
        let that = this;
        let data = that.state.data;
        fetch(globals.url.url + '/hr/employeeType/list', {
            method: 'get',
            credentials: 'include',
        })
    ...
    handleCancel = () => {
        ...
        form.validateFields((err, values) => {
            ...
            if (DataType.type == 'add') {
                url = '/hr/employeeType/add';
            } else {
                url = '/hr/employeeType/update';
            }
    ...
    render() {
        ...
        const columns = [
            ...
            { title: '员工类型', dataIndex: 'name', width: 250, key: 'name' },
